...
<script>
	...
	export default {
		...
		onLoad(option) { 
			this.outMasterId = option.outMasterId;
		...
		methods: {  
			getPickTask(boxCode){
				...
				that.loading = true;
				request({ url: '/wcs-out/PickTask/findByBoxAndOutMasterId', method: 'get', params: { "boxCode": boxCode,"outMasterId":this.outMasterId } }).then(response => {
