<template>
	<view  >
		...
		<view  > 
			<view  @click="toPick(param.waveId,row.boxCode)" style="width:90%;height:auto;padding:10px;background-color:#FFFFFF;margin: 10px;border-radius: 5px;" v-for="row in rows">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.param.waveId = option.waveId;
			this.getRows();
		...
		methods: {  
			toPickByBoxCode(boxCode){
				...
				for(var i=0;i<this.rows.length;i++){
					if(this.rows[i].boxCode == boxCode){
						this.toPick(this.param.waveId,boxCode);
					}
					
				}
				
				uni.navigateTo({
					url: '/pages/work/out/wavePick/wavePick?waveId='+waveId+"&boxCode="+boxCode
				});
			},
			toPick(waveId,boxCode){
				uni.navigateTo({
					url: '/pages/work/out/wavePick/wavePick?waveId='+waveId+"&boxCode="+boxCode
				});
			},
			getRows(){
				...
				request({ url: '/wcs-out/PickTask/findBoxInfoByWaveId', method: 'get', params: this.param }).then(response => {
					that.loading = false;
