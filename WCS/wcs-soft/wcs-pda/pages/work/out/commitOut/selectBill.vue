<template>
	<view  style="padding: 5px;" >
		...
		<view style="margin:10px 0px 10px 10px">
			...
			<uni-easyinput type="text" v-model="param.billNo" placeholder="单据编号" />
			<uni-easyinput type="text" v-model="param.boxCode" placeholder="托盘码" />
			<uni-easyinput type="text" v-model="param.cellCode"  placeholder="货位号" />
			<uni-easyinput type="text" v-model="param.productCode"  placeholder="产品编码" />
		...
		<view  >
			<view  @click="toPick(row.outMasterId)"  style="width:90%;height:auto;padding:10px;background-color:#FFFFFF;margin: 10px;border-radius: 5px;" v-for="row in rows">
				 ...
				 <div class="nameValue">
				  ...
				  <div class="nameValue">
						 <div class="name" style="">客户名称:</div>
						 {{row.customerName}}
				   </div>
				   <div class="nameValue">
						 <div class="name" style="">联系人:</div>
						 {{row.customerLinkMan+"  "}}  {{row.customerLinkPhoneNo}}
				    </div> 
					 <div class="nameValue">
						 <div class="name" style="">地址:</div>
						 {{row.customerAddress}}
					  </div>
					  <div class="nameValue">
						 <div class="name" style="">发运方式:</div>
						 {{row.shipCode}}
					   </div> 
					   <div style="width:95%;
					   background-color:gainsboro;
					   padding: 5px;border-style: solid;border-radius: 5px;border-color:darkgray;margin: 5px;" v-for="outDetail in row.list"> 
					   {{outDetail.productName}}*{{outDetail.quantity}}
					   </div>
					   <button class="button" style="width:90%;margin-left:5%; "size="mini" type="primary"  @click="rows=[];getRows()">发运</button>
	...
	export default {
		...
		methods: {  
			...
			getRows(){
				...
				request({ url: '/wcs-out/OutMaster/list', method: 'get', params: this.param }).then(response => {
					that.loading = false;
