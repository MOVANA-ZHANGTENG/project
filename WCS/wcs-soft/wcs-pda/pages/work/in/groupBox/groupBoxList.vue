<template>
	<view  style="padding: 5px;" >
		...
		<view style="margin:10px 0px 10px 10px">
			<uni-easyinput type="text" v-model="param.twoLevelCode" placeholder="箱号" />  
		...
		<view  >
			<view   style="width:90%;height:auto;padding:10px;background-color:#FFFFFF;margin: 10px;border-radius: 5px;" v-for="row in rows">
				  <div class="nameValue">
					 ...
					 <div class="nameValue">
					 	 <div class="name" style="">时间:</div>
					 	 {{row.createTime}}
					  </div> 
					  <div class="nameValue">
					  	 <div class="name" style="">箱号:</div>
					  	 {{row.twoLevelCode}}
					   </div>
					   <div class="nameValue">
					   	 <div class="name" style="">托盘编码:</div>
					   	 {{row.boxCode}}
					    </div> 
						<div class="nameValue">
							 <div class="name" style="">箱数:</div>
							 {{row.boxCount}}
						 </div> 
	...
	export default {
		...
		methods: {  
			...
			getRows(){
				...
				request({ url: '/wcs-in/GroupBox/list', method: 'get', params: this.param }).then(response => {
					that.loading = false;
