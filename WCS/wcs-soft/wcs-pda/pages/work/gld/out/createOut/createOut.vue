<template>
	<view>
		...
		<!-- outMaster -->
		<uni-card>
			<uni-forms :modelValue="form.outMaster">
				<uni-forms-item label="货主" name="name">
					<uni-data-select v-model="form.outMaster.deptId" @change="changeDept"
						:localdata="deptOptions"></uni-data-select>
					<!-- <uni-easyinput type="text" v-model="form.outMaster.deptId" placeholder="请选择货主" /> -->
				</uni-forms-item>
				<uni-forms-item label="出库单类型" name="memo">
					<uni-data-select v-model="form.outMaster.type" :localdata="outTypes"></uni-data-select>
				</uni-forms-item>
				<uni-forms-item label="备注" name="memo">
					<uni-easyinput type="text" v-model="form.outMaster.memo" placeholder="请输入备注" />
				</uni-forms-item>
				<uni-forms-item label="预计出库日期" name="planningTime">
					<uni-datetime-picker type="date" v-model="form.outMaster.planningTime" @change="changeLog" />
				</uni-forms-item>
			</uni-forms>
		</uni-card>
		...
		<uni-card>
			<uni-card
				...
				v-for="(detail, index) in form.outDetails">
				<uni-row class="demo-uni-row">
					<uni-col :span="22">
						...
						<div class="nameValue">
							<uni-data-select placeholder="请选择批次" v-model="detail.batchNo"
								:localdata="detail.batchList"></uni-data-select>
						</div>
...
<script>
	...
	export default {
		...
		methods: {
			changeDept(e) {
				this.form.outDetails = [];
			},
			deleteRow(index) {
				this.form.outDetails.splice(index, 1);
			},
			addDetail(row) {
				...
				var batchList = [];
				for (var i = 0; i < row.batchList.length; i++) {
					batchList.push({
						value: row.batchList[i],
						text: row.batchList[i]
					})
				}
				this.form.outDetails.push({
			...
			showItemSelect() {
				if (this.form.outMaster.deptId == null) {
					uni.showToast({
						title: '请先选择货主',
						icon: 'error'
					});
				...
				this.item.param.deptId = this.form.outMaster.deptId;
				this.getItemInfos();
			},
			closeItemSelect() {
			...
			getOutTypes() {
				request({
					url: "/system/dict/data/type/out_type",
					method: "get",
					params: {},
				}).then((response) => {
					if (response.code == 200) {
						...
						for (var i = 0; i < outTypes.length; i++) {
							this.outTypes.push({
								value: outTypes[i].dictValue,
								text: outTypes[i].dictLabel,
							});
						}
					} else {
						uni.showToast({
			...
			save() {
				if (this.form.outDetails.length == 0) {
					uni.showToast({
						title: '请选择物料后再提交',
						icon: 'error'
					});
				...
				for (var i = 0; i < this.form.outDetails.length; i++) {
					var detail = this.form.outDetails[i];
