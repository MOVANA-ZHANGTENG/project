...
public class LgNaTask {
    ...
    private String wareCode = "LG-NA";


    private static final Logger log = LoggerFactory.getLogger(LgNaTask.class);
    ...
    public void autoRun1() {
        ...
        for(ProPositionContent content:list){
            ...
            try {
                ...
                if(hasPallet==1 && (  palletCode.trim().equals(""))){
                    content.setMemo("光电感应到，但是为扫到码");
                    content.setIsOk(0);
                }
                if(hasPallet==1 && palletCode.equals("")){
                    content.setIsOk(0);
                    content.setMemo("未扫描到托盘");
                }

                String aaa[]=palletCode.split(";");
                if(aaa.length>1 && (!content.getItemCode().equals(aaa[0]))){
                    content.setIsOk(0);
                    content.setMemo("物料不匹配");
                }
                if(aaa.length>1 && (content.getItemCode().equals(aaa[0])))
                {
    ...
    private PositionInfo getType1(ProPositionContent toContent){
        ...
        try {
            ...
            if(backCode!=null && !backCode.trim().equals("")){
                ...
                ProPositionContent fromContent = proPositionContentService.findByCode(wareCode,backCode);
                 if(fromContent!=null && fromContent.getItemCode().equals(toContent.getItemCode())){

                     PositionInfo positionInfo = positionInfoService.findByCode(wareCode,backCode);
