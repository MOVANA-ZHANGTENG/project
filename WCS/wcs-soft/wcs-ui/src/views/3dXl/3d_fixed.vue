...
export default {
  ...
  methods: {
    ...
    applyCustomFloorMaterials() {
      ...
      // 为每层地板创建深灰色纯色材质（第1层不透明，第2、3、4层透明）
      floorNames.forEach((floorName, index) => {
        ...
        if (floor) {
          ...
          const customMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color(0.15, 0.15, 0.15), // 深灰色 
            specular: new THREE.Color(0, 0, 0), // 无高光反射
            shininess: 0, // 无光泽
            transparent: isTransparent, // 第2、3、4层透明
            opacity: isTransparent ? 0.5 : 1.0, // 透明层使用0.5透明度，不透明层使用1.0
            wireframe: false,
          ...
          console.log(`已为 ${floorName} 应用${isTransparent ? '透明' : '不透明'}深灰色纯色地板材质`);
        } else {
    ...
    applyShelfMaterials() {
      ...
      shelfNames.forEach((shelfName, index) => {
        ...
        if (shelf) {
          // 创建灰白色哑光材质，几乎不反光
          const customMaterial = new THREE.MeshPhongMaterial({
            color: new THREE.Color(0.85, 0.85, 0.85), // 灰白色
          ...
          console.log(`已为 ${shelfName} 应用灰白色哑光材质`);
        } else {
