        ...
        <el-col v-if=" nowPositionData!=null &&  nowPositionData.code!=null" :span="6">
          <el-card style="margin: 3%">
            ...
            <el-descriptions  :column="1" border>
                      ...
                      <el-descriptions-item v-if="nowPositionData.type==2 || nowPositionData.type==3" label="等待点位">
                        <el-input  @change="update(nowPositionData)"  v-model="nowPositionData.code2" placeholder="code2" />
                      </el-descriptions-item>
                      <!-- <el-descriptions-item v-if="nowPositionData.type==1" label="取货点">
          ...
          <el-card style="margin: 3%">
            ...
            <el-descriptions :column="1" border>
              <el-descriptions-item label="托盘状态（测试使用）">
                        <el-select @change="update(nowPositionData)" v-model="nowPositionData.palletState" placeholder="">
                              <el-option
                                style="color: #909399;"
                                key=0
                                label="空"
...
export default {
  ...
  methods: {
    ...
    createGo(wareCode){
      ...
      if(that.myDiagram!=null && that.myDiagram["wareCode"]==wareCode){
        return;
      ...
      if (that.myDiagram) {
        ...
        // 释放Diagram资源
        //that.myDiagram.stopTransaction();
        // that.myDiagram.model = null; 
      }
      that.cellInfos=[];
      that.updateCellQueue=[]; 
      console.log("wareInfo.code",wareInfo.code);
      console.log("wareInfo.backgroundImg",wareInfo.backgroundImg);
      that.init(wareInfo);
      setTimeout(
            function(){ 
              that.myDiagram["wareCode"]=wareCode;
    ...
    submitTaskInfo(taskInfo){
      ...
      taskInfo.type="8";
      taskInfo.wareCode=this.wareInfo.wareCode;
      taskInfo.wareName=this.wareInfo.wareName;
      request({
    ...
    init(wareInfo) {
      ...
      this.myDiagram.add(
        $(go.Part,  // this Part is not bound to any model data
          ...
          $(go.Picture, wareInfo.backgroundImg)
        ));
      ...
      var modelData;
      if (wareInfo.monitorData == null) {
        modelData = {};
      } else { 
        modelData = JSON.parse(wareInfo.monitorData);
        that.loadData(modelData);
      }
         
    //  that.loadData(wareInfo.monitorData);
