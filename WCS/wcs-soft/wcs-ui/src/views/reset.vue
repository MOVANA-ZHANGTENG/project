    ...
    >
      ...
      <el-form-item label="新密码" prop="newPassword">
        <el-input
          ...
          placeholder="8-20位由数字、大小写字母、符号组成"
          type="password"
          show-password
        />
      </el-form-item>
    ...
    return {
      ...
      resetRules: {
        ...
        newPassword: [
          ...
          {
            pattern:
              "/^([a-zA-Z]+[0-9]+[,._!@#$%^&*]+)|([a-zA-Z]+[0-9]+[,._!@#$%^&*]+)|([a-zA-Z]+[,._!@#$%^&*]+[0-9]+)|([0-9]+[,._!@#$%^&*]+[a-zA-Z]+)|([0-9]+[a-zA-Z]+[,._!@#$%^&*]+)|([,._!@#$%^&*]+[a-zA-Z]+[0-9]+)|([,._!@#$%^&*]+[0-9]+[a-zA-Z]+)$/",
    ...
    handleReset() {
      this.$refs.resetForm.validate((valid) => {
        if (valid) {
          ...
          updateUserPwd(this.resetForm.oldPassword, this.resetForm.newPassword)
            .then((res) => {
              this.loading = false;
              if (res.code == 200) {
                this.$alert(
                  "<font color='red'>修改成功，请重新登录</font>",
                  "系统提示",
                  {
