...
export default {
  ...
  methods: {
    ...
    async loadElevatorStatus() {
      // 防止重复请求
      if (this.isUpdatingElevatorStatus) {
        return null;
      }

      this.isUpdatingElevatorStatus = true;
      ...
      try {
        ...
        // 后端返回的是 floor（层数），需要转换为前端需要的格式
        const floor = response.data;

        if (floor == null) {
          console.warn('提升机层数为空');
          return null;
        }

        // 根据层数计算对应的库位编码
        // 1-8-3 (第1层), 2-8-3 (第2层), 3-8-3 (第3层)
        let cellCode = null;
        if (floor >= 1 && floor <= 3) {
          cellCode = floor + "-8-3";
        }
        ...
        this.isUpdatingElevatorStatus = false;
      }
    },

    // 启动提升机状态更新定时器
    startElevatorStatusUpdate() {
      // 如果定时器已存在，先清除
      if (this.elevatorStatusTimer) {
        clearInterval(this.elevatorStatusTimer);
      }

      console.log('启动提升机状态更新，每2秒更新一次');

      // 立即执行一次
      this.updateElevatorStatus();

      // 每2秒更新一次
      this.elevatorStatusTimer = setInterval(() => {
        this.updateElevatorStatus();
      }, 1000);
    },

    // 停止提升机状态更新
    stopElevatorStatusUpdate() {
      if (this.elevatorStatusTimer) {
        clearInterval(this.elevatorStatusTimer);
        this.elevatorStatusTimer = null;
        console.log('已停止提升机状态更新');
      }
    },

    // 更新提升机状态并同步四向车
    async updateElevatorStatus() {
      const elevatorStatus = await this.loadElevatorStatus();

      if (elevatorStatus) {
        // 同步四向车跟随提升机移动
        this.syncVehiclesWithElevator(elevatorStatus);
      }
    },
    ...
    init() {
      ...
      loader.load("/glb/智库.glb", function (glb) {
        ...
        // 启动提升机状态更新（每2秒更新一次）
        that.startElevatorStatusUpdate();
