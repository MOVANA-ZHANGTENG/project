...
export class FirstPersonController {
  ...
  onMouseMove(event) {
    ...
    if (Math.abs(movementX) > 2 || Math.abs(movementY) > 2) {
      console.log('🖱️ 鼠标移动:', 'X:', movementX, 'Y:', movementY);
    }
   ...
   */
  onPointerLockChange() {
    console.log('🔔 指针锁定状态变化事件触发');
    console.log('  pointerLockElement:', document.pointerLockElement);
    console.log('  期望的domElement:', this.domElement);
    console.log('  是否匹配:', document.pointerLockElement === this.domElement);
    
    if (document.pointerLockElement === this.domElement) {
      this.isLocked = true;
      console.log('✅ 🔒 鼠标已成功锁定, isLocked =', this.isLocked);
      ...
      this.isLocked = false;
      console.log('❌ 🔓 鼠标已解锁或锁定失败, isLocked =', this.isLocked);
  ...
  lock() {
    console.log('📌 请求指针锁定...');
    console.log('  目标元素:', this.domElement);
    console.log('  元素ID:', this.domElement?.id);
    ...
    try {
      ...
      console.log('✅ requestPointerLock已调用，请点击画面确认...');
    } catch (error) {
  ...
  enable() {
    ...
    console.log('✅ 鼠标事件已绑定（指针锁定模式）');
    
    // 自动请求指针锁定
    setTimeout(() => {
      this.lock();
      console.log('📌 正在请求指针锁定，请点击画面确认...');
    }, 500);
    
    console.log('🚁 无人机飞行模式已启用（FPS风格）');
    console.log('操作说明：');
    console.log('- 🖱️ 点击画面锁定鼠标');
    console.log('- 🖱️ 移动鼠标控制视角（准星固定在中心）');
    console.log('- ⌨️ 方向键/WASD：控制移动');
    console.log('  · ↑/W：前进');
    console.log('  · ↓/S：后退');
    console.log('  · ←/A：向左平移');
    console.log('  · →/D：向右平移');
    console.log('- ⬆️ Space：上升 | ⬇️ Shift：下降');
    console.log('- 🔓 ESC：解锁鼠标');
  }
  ...
  toggleAutoForward() {
    ...
    console.log(`自动前进: ${this.autoForward ? '启用' : '禁用'}`);
  ...
  setMouseSensitivity(sensitivity) {
    ...
    console.log(`鼠标灵敏度已设置为: ${sensitivity}`);
  }
  ...
  setRotationSmoothing(smoothing) {
    ...
    console.log(`旋转平滑度已设置为: ${smoothing}`);
  }
}
