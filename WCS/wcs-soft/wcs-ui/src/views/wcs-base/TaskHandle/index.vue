<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="任务定义ID" prop="taskDefineId">
        <el-input v-model="queryParams.taskDefineId" placeholder="请输入任务定义ID" clearable
      ...
      <el-form-item label="排序" prop="index">
        <el-input v-model="queryParams.index" placeholder="请输入排序" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="创建人" prop="createUserId">
        <el-input v-model="queryParams.createUserId" placeholder="请输入创建人" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="更新人" prop="updateUserId">
        <el-input v-model="queryParams.updateUserId" placeholder="请输入更新人" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['wcs-base:TaskHandle:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['wcs-base:TaskHandle:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['wcs-base:TaskHandle:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['wcs-base:TaskHandle:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="TaskHandleList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" align="center" prop="id" />
      <el-table-column label="任务定义ID" align="center" prop="taskDefineId">
    ...
    <el-dialog v-dialogDrags :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="任务定义ID" prop="taskDefineId">
          <el-input v-model="form.taskDefineId" placeholder="请输入任务定义ID" />
        </el-form-item>
        <el-form-item label="执行器ID" prop="handleId">
          <el-input v-model="form.handleId" placeholder="请输入执行器ID" />
        </el-form-item>
        <el-form-item label="类型 0-cmd_pre  1-cmd 2-success_re 3-success" prop="type">
...
<script>
import { listTaskHandle, getTaskHandle, delTaskHandle, addTaskHandle, updateTaskHandle } from "@/api/wcs-base/TaskHandle";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTaskHandle(this.queryParams).then(response => {
        this.TaskHandleList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTaskHandle(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTaskHandle(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTaskHandle(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除任务执行器编号为"' + ids + '"的数据项？').then(function () {
        return delTaskHandle(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wcs-base/TaskHandle/export', {
        ...this.queryParams
      }, `TaskHandle_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
