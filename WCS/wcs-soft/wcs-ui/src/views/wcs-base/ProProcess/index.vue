    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['wcs-base:ProProcess:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['wcs-base:ProProcess:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['wcs-base:ProProcess:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['wcs-base:ProProcess:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="ProProcessList" @selection-change="handleSelectionChange" border>
...
<script>
import { listProProcess, getProProcess, delProProcess, addProProcess, updateProProcess } from "@/api/wcs-base/ProProcess";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listProProcess(this.queryParams).then(response => {
          if(response.code==200){
            this.ProProcessList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getProProcess(id).then(response => {
          if(response.code==200){
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateProProcess(this.form).then(response => {
             if(response.code==200){
            ...
            addProProcess(this.form).then(response => {
             if(response.code==200){
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wcs-base/ProProcess/export', {
        ...this.queryParams
      }, `ProProcess_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
